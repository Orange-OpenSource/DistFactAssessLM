...
import pandas as pd
from glob_core import Mongoable, TimeUnit
from kb.core import Date, Interval, Proposition, Quantity, Relation, String, Triple, Entity, TripleComp
from utils.general import PrintableException, load_json
from verb.options import DateIndicator, DatePrecision, Order, PeriodIndicator, TempIndicator, VerbalizeOption, Tense, BlankOut, QuantityStyle, QuantityPrecision, DateStyle
import spacy
...
class EntityFormatter:
    ...
    def format_date(self, date : Date) -> str:
        ...
        assert formatted_date is not None, "Date could not be verbalized (Date=%s, level=%s, config.date_precision=%s, config.date_style=%s)" % (date, date.level, date_precision, date_style)
...
class VerbalizeConfig(Mongoable):
    max_num_verbs : int = 5 # The maximal number of verbalizations to produce per triple
    verb_tense : Tense = Tense.PRESENT # Verbalize in present tense. If None, keep the tense of the original template.
    temporal_indicator : TempIndicator = None # No temporal indicator used
    date_precision : DatePrecision = None # Take full date
    date_style : DateStyle = DateStyle.MONTH_LETTER_CLASSIC # Classic way of representing dates (e.g. 1 February 2020)
...
class TemplateVerbalizer(Verbalizer):
    ...
    def __init__(self, template_scores : dict[Relation, dict[Template, int]]) -> None:
        ...
        Args:
            template_scores (dict[Relation, dict[Template, int]]): The quality score of each template grouped by relations
        """
    ...
    def verbalize(self, triple : Triple, config : VerbalizeConfig = None, skip_failed_conjugation = False) -> list[Template]:
        """Verbalize a triple
        ...
        templates = self._rel2temp.get(triple.relation, None)
        if templates is None:
            exc = TemplateNotFound("The templates for relation %s could not be found in TemplateVerbalizer" % triple.relation)
            if skip_failed_conjugation:
                print(exc)
                return []
            else:
                raise exc
        templates = [temp.copy() for temp, _ in templates]
        ...
        n_verbs = config.max_num_verbs if config.max_num_verbs is not None else len(templates)
        ...
        for temp in templates:
            ...
            if isinstance(config.temporal_indicator, DateIndicator):
                ...
                date_txt = DEFAULT_ENTITY_FORMATTER(config.temporal_indicator.date)
...
class Template(Mongoable):
    ...
    def inject(self, sub : Entity, obj : Entity):
