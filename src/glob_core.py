...
from utils.general import concat, dotdict, dump_json, get_class, get_mongodb_client, load_json
    ...
    class BypassOverridence(type):
        ...
        def __new__(mcs, name, bases, namespace):
            ...
            if function_name not in namespace:
                setattr(cls, function_name, getattr(BypassOverridence.base_cls, function_name))
            return cls
    if add_abc:
...
class Blueprint(metaclass=ABCMeta):
    ...
    def build(self, force=False, confirm=True) -> None:
        ...
        if confirm:
            while True:
                ans = input("This process will possibly take a long time to finish. Are you sure you want to continue? [Y/n]")
                ans = ans.lower().strip()
    ...
    def destroy(self, force=False, confirm=True) -> None:
        ...
        if confirm:
            while True:
                ans = input("You are about to delete this object : %s. Are you sure you want to continue? [Y/n]" % self)
                ans = ans.lower().strip()
                if ans in ('y', 'yes') or ans.strip() == '':
                    break
                elif ans in ('n', 'no'):
    ...
    def _replace_json_buildings(data):
        ...
        if isinstance(data, dict):
            ...
            if replace_with == 'json':
                return data._to_full_json(single_json=True, include_idtime=False)[0]
            else:
                to_build.append(data)
                return JSON_ID_INDICATOR + ":" + data._hash()
        elif issubclass((t := type(data)), JSONABLE_TYPES):
            d, _ = cls2saveablecls(t)(data)._to_full_json(single_json=True, include_idtime=False)
            d['resolve'] = True
            return d
        else:
            return data
...
class JSONable:
    ...
    def _to_json(self) -> dict | list:
        ...
        raise NotImplementedError("Please implement _to_json for the class %s" % self.__class__)
    ...
    @classmethod
    def _hash_class(cls, id : Any) -> str:
        cls_id = cls.class_id()
        id, _ = replace_json_buildings(id)
        return sha256(pickle.dumps((id, cls_id))).hexdigest()
    
    def _hash(self) -> str:
        cls = self.__class__
        return cls._hash_class(self._identifier())
    
    def _to_full_json(self, single_json=True, include_idtime=True) -> tuple[list|dict, list[JSONable]]:
        d = self._to_json()
    ...
    @classmethod
    def _from_path(cls : Type[SameType], path : str | None, ignore_not_found=False) -> SameType:
        if path is not None:
            json_dict = load_json(path)
            return cls._from_full_json(json_dict)
        if not ignore_not_found:
            cls_id = cls.class_id()
            raise SaveableNotFoundError("Object instance of %s with path=%s could not be found!" % (cls_id, path))
        return None
        

    @classmethod
    def from_id(cls : Type[SameType], id : Hashable, many=False, ignore_not_found=False) -> SameType | list[SameType]:
        if many:
            res = []
            for id_ in id:
    ...
    def size_in_bytes(self) -> int:
        ...
        if path is None:
            raise SaveableNotFoundError('This JSON building was not found on the hard drive!')
        return osp.getsize(path)

class Mongoable(JSONable):
    # Specify custom MongoDB URL in the next line if you want
    MONGO_COLLECTION = get_mongodb_client(mongodb_url=None).get_database('buildings').get_collection('mongobuildings')
    MONGO_COLLECTION.create_index("class")
    
    @classmethod
    def _save_full_json(cls, full_json: dict) -> None:
        cls.MONGO_COLLECTION.replace_one({'_id' : full_json['_id']}, full_json, upsert=True)
        

    @classmethod
    def from_id(cls : Type[SameType], id : Hashable | Iterable[Hashable], many=False, ignore_not_found=False) -> SameType | list[SameType]:
        if many:
            res = cls._from_id_hash([cls._hash_class(id_) for id_ in id], ignore_not_found)
            return res
    ...
    @classmethod
    def _from_id_hash(cls : Type[SameType], id_hash : str | list[str], ignore_not_found=False) -> SameType | list[SameType]:
        if isinstance(id_hash, list):
            documents = {x['_id'] : x for x in cls.MONGO_COLLECTION.find({'_id' : {'$in' : id_hash}})}
            results = []
            for id in id_hash:
...
class Cache(Mongoable):
    MONGO_COLLECTION = get_mongodb_client(mongodb_url=None).get_database('buildings').get_collection('cache')
    MONGO_COLLECTION.create_index("class")
    data : Any
    identifier : Any
    function : str
...
class NumpyArray(JSONableData):
    ...
    def _to_json(self) -> dict | list:
        ...
        return {'bytes' : base64.b64encode(b), 'shape' : self.array.shape, 'dtype' : self.array.dtype.name}
    
    @classmethod
    def _from_json(cls: type[SameType], d: dict | list) -> SameType:
        array = np.frombuffer(base64.b64decode(d['bytes']), d['dtype']).reshape(d['shape'])
        return cls(array)
...
def get_cache_by_class(cls : type[JSONable]) -> list[Cache]:
    l = [Cache._from_full_json(x).get_value() for x in Cache.MONGO_COLLECTION.find({'class' : Cache.class_id()})]
...
def cache_return(func : Callable, args_identifier : tuple[str], loop_args : list[str] = [], additional_kwargs : dict = {}, postprocess : Callable = lambda x:x, verbose=False) -> Callable:
    @wraps(func)
    def new_func(*args, **kwargs):  
        if is_cache_activated(func):
            ...
            identifier_short = SortedDict({k: v._to_full_json(include_idtime=False) if isinstance(v, JSONable) else v for k,v in identifier.items()})
            ...
            identifier_short = [identifier_short.copy() for _ in range(len(loops[0]) if len(loops) else 1)]
            for j, arg in enumerate(loop_args):
                for i, id_ in enumerate(identifier_short):
                    id_[arg] = loops[j][i]
            cache = list(map(postprocess, Cache.from_id(({'id': x, 'func' : func.__qualname__} for x in identifier_short), many=True, ignore_not_found=True)))
            ...
            results = [None if x is None else x.get_value() if isinstance(x, JSONableData) else x for x in results]
