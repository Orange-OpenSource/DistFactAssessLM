...
df_with_all_columns = df.withColumn("all_columns", struct([df[col] for col in df.columns]))
...
for index, row in tfidf_vectors.collect():
    ...
    vector = {int(i): float(x) for i, x in zip(row['features'].indices, row['features'].values)}

    # Create the Qdrant point
    point = PointStruct(payload={}, vector=vector)

    # Add the point to Qdrant
    qdrant_client.points.replace(point_id=index, points=[point], collection_name="your_collection_name")
