...
import sys
from _old_codebase.build.utils.wd import db, ent_to_vec, wikidata_iterator
from _old_codebase.build.utils.general import dump_json
from _old_codebase.build.gpt3_5_verbalization.verbalize_wikidata import run_in_process
from sklearn.feature_extraction.text import TfidfVectorizer

import nmslib
import scipy.io
from _old_codebase.build.find_neighbors.config import TFIDF_FULL_INDEX_FOLDER, TFIDF_WIKIPEDIA_INDEX_FOLDER
...
def main(full=False, time='old', add_rev_info=False):
    ...
    features_sparses_0, features_sparses_1 = features_sparses[:version_count[0]], features_sparses[version_count[0]:]
    entities_0, entities_1 = list_seen_entities[:version_count[0]], list_seen_entities[version_count[0]:]
    ...
    scipy.sparse.save_npz(osp.join(save_folder, 'features_sparses_%s.npz' % order[0]), features_sparses_0)
    scipy.sparse.save_npz(osp.join(save_folder, 'features_sparses_%s.npz' % order[1]), features_sparses_1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--full', action='store_true', help="Compute embeddings for all Wikidata entities. WARNING : Do not select this option unless you have >400GB of RAM")
    parser.add_argument('--time', type=str, choices=['old', 'new'], help="It can take two values : 'old' which builds a KNN at time=old and 'new' which builds a KNN at time=which builds a KNN at time=new. Defaults to 'old'", default='old')
    parser.add_argument('--add_rev_info', action='store_true', help="Add reverse information in entity vectors, i.e., if the script finds the triple (s,r,o), it adds s,r, and (s,r) to the features of o. It is deactivated by default.")
