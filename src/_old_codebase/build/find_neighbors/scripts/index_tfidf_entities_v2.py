...
import nmslib
import scipy.io
from _old_codebase.find_neighbors.config import TFIDF_FULL_INDEX_FOLDER, TFIDF_WIKIPEDIA_INDEX_FOLDER
...
def generator_full(version : str, seen_entities : set[str]):
    ...
    for x in tqdm.tqdm(db['wikidata_%s_json' % version].find({}, {'claims' : 1}).limit(100), total=total, mininterval=5, file=sys.stdout):
        if x['_id'] in seen_entities:
            continue
        yield x['_id'], x['claims']
...
def main(full=False):
    ...
    count_vect = CountVectorizer(analyzer=identity, vocabulary={k:i for i,k in enumerate(vocabulary)})
    features = count_vect.fit_transform(df['Features'].to_bag())
    idf = array.log((features.shape[0]+1) / (features.sum(axis=1, keepdims=True) + 1)) + 1
    tf_idf = features * idf
    ...
    scipy.sparse.save_npz(osp.join(save_folder, 'features_sparses_old.npz'), features_sparses_old)
    scipy.sparse.save_npz(osp.join(save_folder, 'features_sparses_new.npz'), features_sparses_new)
