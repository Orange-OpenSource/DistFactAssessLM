...
import numpy as np
from _old_codebase.build.find_neighbors.config import TFIDF_WIKIPEDIA_INDEX_FOLDER, TFIDF_FULL_INDEX_FOLDER
from _old_codebase.build.find_neighbors.scripts.index_tfidf_entities import main as main_nmslib_index

import os.path as osp
import nmslib
import scipy.sparse

from _old_codebase.build.utils.general import load_json
...
class KNearestNeighbors:
    def __init__(self, full=False, time='old', load_vocab : bool = False, add_reverse_features = False) -> None:
        ...
        self.full = full
        self.time = time
        self.location = TFIDF_WIKIPEDIA_INDEX_FOLDER if not self.full else TFIDF_FULL_INDEX_FOLDER
        if self.time == 'new':
            self.location += '_new'
    ...
    def load_index(self):
        ...
        self.entities_in_index = read_list(osp.join(self.location, 'entities_%s.txt' % order[0]))
        self.entities_other = read_list(osp.join(self.location, 'entities_%s.txt' % order[1]))
        ...
        self.tfidf_old = scipy.sparse.load_npz(osp.join(self.location,'features_sparses_old.npz'))
        self.tfidf_new = scipy.sparse.load_npz(osp.join(self.location,'features_sparses_new.npz'))

        self.order = (('old', self.entities_in_index_name2idx), ('new', self.entities_other_name2idx)) if self.time == 'old' else (('new', self.entities_in_index_name2idx), ('old', self.entities_other_name2idx))
        if self.load_vocab:
            self._load_vocab()
...
if __name__ == '__main__':
    from _old_codebase.build.utils.wd import get_info_wikidata
