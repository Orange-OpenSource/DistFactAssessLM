...
class Task:
    ...
    def run(self):
        ...
        try:
            with open(osp.join(progress_folder, self.format_filename() + '.success')) as f:
...
tasks = [
    ...
    ('Collect aliases (Old Wikidata)', 'build/wikidata_scripts/collect_aliases.py --version old', ['Preprocess Old Wikidata']),
    ('Collect aliases (New Wikidata)', 'build/wikidata_scripts/collect_aliases.py --version new', ['Preprocess New Wikidata']),
    ...
    ('Verbalize WikiFactDiff + KNT', 'build/verbalize_wikifactdiff/verbalize_wikifactdiff.py --ann_method sparse --force_reset', ['Create WikiFactDiff (Triples)', 'Setup K-Nearest-Triples (KNT)', "Collect aliases (Old Wikidata)"]),
    ('Clean WikiFactDiff', 'build/wikidata_scripts/clean_wikifactdiff.py', ['Verbalize WikiFactDiff + KNT'])
]

class DockLayoutExample(App):
    CSS_PATH = osp.join(root_folder, 'src/_old_codebase/build/style.tcss')
    refresh_timer : Timer
    exit_timer : Timer
    animation_timer : Timer
    # BINDINGS = [("q", "quit", "Quit")]

    def __init__(self, task_launcher : TaskLauncher, tasks, driver_class: type[Driver] | None = None, css_path: str | PurePath | List[str | PurePath] | None = None, watch_css: bool = False):
    ...
    def compose(self) -> ComposeResult:
        with Horizontal():
            with RadioSet(id="sidebar"):
                yield RadioButton(self.tasks[0][0], value=True, id=self.tasks[0][0])
    ...
    def exit_when_finished(self):
        if all(x.finished for x in self.task_launcher.tasks) and all(not th.is_alive() for th in self.task_launcher.threads):
    ...
    def refresh_animation(self):
        for radio in self.query(RadioButton):
            task = self.task_launcher.name2task[radio.id]
            if task.waiting:
...
if __name__ == "__main__":
    from _old_codebase.build.config import OLD_WIKIDATA_DATE, NEW_WIKIDATA_DATE, STORAGE_FOLDER
    import os.path as osp
    import os
    import json
