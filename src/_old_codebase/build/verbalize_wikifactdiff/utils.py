...
from _old_codebase.build.gpt3_5_verbalization.utils import KnowledgeTriple, Entity, Literal, Property
...
DEFAULT_CHATGPT_VERBALIZATIONS_RAW_THEN_TEMPLATES_PATH = osp.join(STORAGE_FOLDER, "chatgpt_wfd_verbalization")
...
class Verbalizer:
    def __init__(self, verbalization_path = None) -> None:
        ...
        best_templates_reverse_path = osp.join(verbalization_path, 'best_templates_reverse.json')
        ...
        for x in iterator_verbalization_files(verbalization_path):
            ...
            verbs = [get_fill_in_the_blank(y, object_label) for y in x['verbalizations']]

            verbs = [blank_out(y, subject_label) for y in verbs if y is not None]
            verbs = [y for y in verbs if y is not None]

            reverse_verbs = [get_fill_in_the_blank(y, subject_label) for y in x['verbalizations']]
            reverse_verbs = [blank_out(y, object_label) for y in reverse_verbs if y is not None]
        ...
        self.best_reverse_templates = {prop_id: Counter(verbs).most_common(5) for prop_id, verbs in prop_rev_fitb.items()}

        if not osp.exists(best_templates_reverse_path):
            json.dump(self.best_templates, open(best_templates_path, 'w'))

        if not osp.exists(best_templates_reverse_path):
            json.dump(self.best_reverse_templates, open(best_templates_reverse_path, 'w'))
    
    def verbalize(self, triple : KnowledgeTriple, exclude : Union[str, list[str]] = [], reverse=False, return_formatted_subject_and_object=False) -> list[str]:
        ...
        best_templates = self.best_templates if not reverse else self.best_reverse_templates
