...
from _old_codebase.build.utils.wd import db, get_info_wikidata, get_value, get_single_valued_properties
from _old_codebase.build.gpt3_5_verbalization.verbalize_wikidata import PropertyInfoRetriever, Entity, Property, Literal, KnowledgeTriple
from typing import Union
from _old_codebase.build.utils.ku import Feature, classify_algorithm_lite
from _old_codebase.build.verbalize_wikifactdiff.utils import Verbalizer
...
def get_same_entity_triples(ent_id : str, rel_to_exclude : Property, ent_is_ph_new : bool, k=5) -> list[KnowledgeTriple]:
    if ent_is_ph_new:
        return []
    ent_dict = db['wikidata_old_prep'].find_one({'_id' : ent_id}, {'claims' : 1, 'label' : 1, 'description' : 1})
    if ent_dict is None:
        return []
    ...
    for prop_id, snaks in (random.sample(closes, k=k) if len(closes) > k else closes):
        ...
        objects = [value_to_object(snak['mainsnak']['datavalue']['value']) for snak in snaks]
...
def verbalize_and_compact(verbalizer : Verbalizer, triples_decisions : list[tuple[KnowledgeTriple, str]], neighbor_triples : list[KnowledgeTriple], distances: list[float], close_triples : list[list[KnowledgeTriple]], ent_is_ph_new : bool, rel_is_functional : bool, ent_imp : float) -> dict:
    ...
    if verbs is None:
        return None
    rev_verbs = verbalizer.verbalize(KnowledgeTriple(triple.object, triple.relation, triple.subject), exclude=['object', 'subject'], reverse=True)
    if rev_verbs:
    ...
    neighbors = [{'objects' : v, 'subject' : k.to_dict(), 'dist' : dist} for k,v in neighbors.items()]
    ...
    closes = [{'relation' : k.to_dict(), 'objects' : v} for k,v in closes.items() if k not in relations_to_skip ]
...
def process_one_group(x):
    ...
    instance = verbalize_and_compact(verbalizer, triples_decisions, neighbor_triples, distances, close_triples, ent_ph_new, rel_is_functional, x['ent_imp'])
    return instance
...
if __name__ == '__main__':
    ...
    if args.ann_method == 'dense':
        from _old_codebase.build.find_neighbors.knn_dense import KNearestNeighbors
    else:
        from _old_codebase.build.find_neighbors.knn_sparse_nmslib import KNearestNeighbors
