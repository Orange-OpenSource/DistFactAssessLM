...
from _old_codebase.build.utils.ku import FeatureDiff, classify_algorithm_full
from _old_codebase.build.utils.wd import db, get_single_valued_properties
...
ph_new_entities = set(x['_id'] for x in db['physically_new_entities__%s__%s' % (args.time_old, args.time_new)].find())
...
for i, ent_dict in enumerate(cursor):
    ...
    if i % 1000 == 0:
        ...
        GlobalStat.save(osp.join(STORAGE_FOLDER, 'resources/script_stats/create_wkd__%s__%s.json' % (args.time_old, args.time_new)))
