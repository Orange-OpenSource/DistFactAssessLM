...
parser.add_argument("--version", choices=['old', 'new'], help='The version of Wikidata from which the script will collect aliases', required=True)
...
if __name__ == '__main__':
    ...
    for i, d in tqdm.tqdm(enumerate(coll.find({}, {'aliases' : 1, 'label' : 1}), start=1), total=coll.estimated_document_count(), mininterval=5, file=sys.stdout):
        id = d.get('_id', None)
        ...
        aliases = [alias for alias, alias_lower in zip(aliases, aliases_lower) if label_lower not in alias_lower and alias_lower not in label_lower and len(alias_lower) > 4] 
        if len(aliases) == 0:
            continue
