...
parser.add_argument("--version", help='Wikidata version to process', type=str, required=False, choices=['old', 'new'])
parser.add_argument("--date", help='Wikidata version to process (select version using date)', type=str, required=False)
parser.add_argument("--collection_name", help='MongoDB collection name', type=str, default=None)
parser.add_argument(
        "--limit",
        type=int,
...
def process_io(queue_io, path, file_position : float, n_cpu : int, n_cpu_process : int, chunk_size : int, limit : int):
...
if __name__ == '__main__':
    ...
    assert date is not None or version is not None, "Atleast one of 'date' or 'time' should be specified!"
    ...
    p_io = mp.Process(target=process_io, args=(io_q, path, file_position, args.n_cpu_bz2_decompression, args.n_cpu_process_chunk, args.chunk_size, limit))
