...
from _old_codebase.build.config import MONGO_URL, MONGODB_NAME, REWIND_N_MONTHS, STORAGE_FOLDER
import numpy as np
import bson
...
def process_io(vars):
    ...
    for i, chunk in enumerate(db["wikidata__%s__PREPROCESSED" % time].find_raw_batches(batch_size=N_DOCUMENT_CHUNK)):
...
def process_chunk(vars):
    ...
    while vars['io_alive'].value or vars['queue_io'].qsize():
        ...
        chunk = list(db['wikidata__%s__ALMOST_RAW' % time].find({'_id' : {'$in' : chunk}, "enwiki_title" : {'$exists' : 1}}, {'enwiki_title' : 1}, batch_size=N_DOCUMENT_CHUNK))
        ...
        ent2types = {}
        consults = list(db['%s_wikipedia_month_consultation' % time].find({'_id' : {'$in' : list(pointers.keys())}}, {'_id' : 1, 'month_consults' : 1}, batch_size=N_DOCUMENT_CHUNK))
        for d in consults:
            ent = pointers[d['_id']]
...
def process_stats(phase_name : str, position : int, n_documents : int, queues_dict : dict[str, mp.Queue]):
    ...
    while True:
        ...
        print('Speed : {} entities/sec'.format(round(position.value/(time_pack.time() - start_time), 2)))
...
def main(time : str):
    ...
    p_stat = mp.Process(target=process_stats, args=('Wikidata %s - Acquiring importance stats (entity types + property)' % time, process_io_vars['position'], n_documents, queue_dict))
    ...
    db['%s_entity_type_property_importance' % time].insert_many(ent_prop_imp, ordered=False)
    del ent_prop_imp
    ...
    with open(osp.join(STORAGE_FOLDER, 'resources/script_stats', osp.splitext(osp.basename(__file__))[0] + '_%s_stats.json' % time), 'w') as f:
        json.dump(stats, f)
    ...
    db['%s_entities_types_importance' % time].insert_many(entities_types_importance_stats, ordered=False)
