...
from _old_codebase.build.utils.wd import db, get_single_valued_properties, get_info_wikidata, get_direct_indirect_types, get_value
from _old_codebase.build.utils.ku import get_start_end_time, get_point_in_time, old_wikidata_timestamp, new_wikidata_timestamp
...
def main(version : str, collection_in : str, collection_out : str, time_ : str):
    ...
    for i, ent_dict in enumerate(cursor, 1):
        ...
        if i % 1000 == 0:
            ...
            GlobalStat.save(osp.join(STORAGE_FOLDER, 'resources/script_stats/preprocess_dump_%s.json' % collection_out))
    ...
    with open(osp.join(STORAGE_FOLDER, 'resources/property_lists/post_preproc_properties_%s.json' % collection_out), 'w') as f:
        json.dump(list(GlobalStat.post_relation_count.keys()), f) 
        
    with open(osp.join(STORAGE_FOLDER, 'resources/property_lists/restrictive_qualifiers_%s.json' % collection_out), 'w') as f:
        json.dump(list(restricted_triplet_det.yes), f)

    with open(osp.join(STORAGE_FOLDER, 'resources/property_lists/meta_properties_%s.json' % collection_out), 'w') as f:
    ...
    with open(osp.join(STORAGE_FOLDER, 'resources/entity_lists/irrelevant_entities_%s.json' % collection_out), 'w') as f:
...
if __name__ == '__main__':
    ...
    t1 = time.time()
    main(version=args.version, collection_in=args.collection_in, collection_out=args.collection_out, time_ = args.time)
