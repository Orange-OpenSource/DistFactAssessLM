...
from _old_codebase.build.utils.ku import correct_time
from _old_codebase.build.utils.wd import db, sim_dict
import os.path as osp
from copy import deepcopy
from _old_codebase.build.utils.ku import old_wikidata_timestamp
import argparse
...
parser.add_argument(
        "--time_old",
        help="Timestamp of the Old Wikidata (Format : YYYYMMDD)",
        default=None
    )
parser.add_argument(
        "--time_new",
        help="Timestamp of the New Wikidata (Format : YYYYMMDD)",
        default=None
    )
...
coll_physically_new_entities = db['physically_new_entities__%s__%s' % (args.time_old, args.time_new)]
...
for i, old_ent_dict in enumerate(old_coll.find(), 1):
    ...
    if i % 1000 == 0:
        ...
        GlobalStat.save(osp.join(STORAGE_FOLDER, 'resources/script_stats/compute_diff__%s__%s.json' % (args.time_old, args.time_new)))
