...
from _old_codebase.build.utils.wd import get_info_wikidata, get_objects_subrel_pair
from _old_codebase.build.utils.ku import new_wikidata_timestamp, old_wikidata_timestamp
from _old_codebase.build.utils.general import subrel2str, uniquifier
import numpy as np
...
def infuse_valid_objects(knn, valid_objects, n_neighbors : int):
    ...
    for i,obj in enumerate(valid_objects):
        ...
        valid_objects[i][('alt_new' if new else 'alt') + ('_revfeat' if rev_feat else '')] = alt_objects
...
def get_temporally_adjacent_objects(subject : str, relation : str, new : bool) -> list:
    ...
    timestamps = np.array([x if not isinstance(x, (list, tuple)) else (x[0] + (x[1] - x[0]) / 2) for x in timestamps])
...
def infuse_eps_enhanced_neighbor_objects(knn, subject : str, relation : str, valid_objects, eps : float, n_neighbors : int):
    new = knn.time == 'new'
    rev_feat = knn.add_reverse_features
    for i,obj in enumerate(valid_objects):
        vect = knn.get_vectors(obj['id'])
        ...
        for a in alt_objects:
            a['label'] = a.pop('name')
        alt_objects = [x for x in alt_objects if len(x['label'])]
        valid_objects[i][f'eps={eps}-enhanced_neigh' + ('_new' if new else '') + ('_revfeat' if rev_feat else '')] = alt_objects
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_custom_templates', action='store_true', help='Use the diverse templates including questions, yes/no questions, and python-style questions')
    parser.add_argument('--n_facts', type=int, help='Number of tested facts. Default 1000', default=1000)
    parser.add_argument('--n_neighbors', type=int, help='Number of neighbors for each fact. Defaults to 100', default=100)
    parser.add_argument('--neigh_method', type=str, help='Choose only one neighbor method. \
                        ...
                        Defaults to None (add all neighbors method)', default=None)
    ...
    if not revfeat or load_all_knns:
        knn_without_rev = KNearestNeighbors(time="old", load_vocab=True, add_reverse_features=False)
        knn_new_without_rev = KNearestNeighbors(time='new', load_vocab=True, add_reverse_features=False)
    ...
    if revfeat or load_all_knns:
        ...
        knn_new = KNearestNeighbors(time='new', load_vocab=True, add_reverse_features=True)
        knn_new.load_index()
        knn.load_index()
    ...
    for x in wfd:
        ...
        try:
            ...
            if len(valid_objects) == 0 or any(y['description'] in ('String', 'Date', 'Year', 'Month', 'Quantity') for y in valid_objects):
                continue
            ...
            if args.neigh_method is None:
                ...
                infuse_valid_objects(knn_new_without_rev, valid_objects, args.n_neighbors)
                x['temp_adj_objects'] = get_temporally_adjacent_objects(subject, relation, new=False)
                x['temp_adj_objects_new'] = get_temporally_adjacent_objects(subject, relation, new=True)
                for eps in (0.001, 0.01, 0.1, 1, 10, 50, 100, 500, 1000):
                    infuse_eps_enhanced_neighbor_objects(knn, subject, relation, valid_objects, eps, args.n_neighbors)
                    infuse_eps_enhanced_neighbor_objects(knn_new, subject, relation, valid_objects, eps, args.n_neighbors)
                    infuse_eps_enhanced_neighbor_objects(knn_without_rev, subject, relation, valid_objects, eps, args.n_neighbors)
                    infuse_eps_enhanced_neighbor_objects(knn_new_without_rev, subject, relation, valid_objects, eps, args.n_neighbors)
            else:
                neigh_sys, neigh_sys_new = (knn, knn_new) if revfeat else (knn_without_rev, knn_new_without_rev)
                infuse_eps_enhanced_neighbor_objects(neigh_sys, subject, relation, valid_objects, chosen_eps, args.n_neighbors)
                infuse_eps_enhanced_neighbor_objects(neigh_sys_new, subject, relation, valid_objects, chosen_eps, args.n_neighbors)
                if temp:
                    x['temp_adj_objects'] = get_temporally_adjacent_objects(subject, relation, new=False)
                    x['temp_adj_objects_new'] = get_temporally_adjacent_objects(subject, relation, new=True)
                if chosen_eps == 0:
    ...
    neigh_method_suffix = ('_' + args.neigh_method) if args.neigh_method is not None else ""
