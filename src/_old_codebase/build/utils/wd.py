...
import tqdm
from _old_codebase.build.config import *
...
from _old_codebase.build.utils.general import run_in_process, subrel2str
from _old_codebase.build.utils.ku import get_point_in_time
from _old_codebase.build.utils.ku import Feature
...
def _validate_version_arg(version : str):
    assert version in ('old', 'new'), "argument version must be one of the following values : ('old', 'new)"
...
from kb.wikidata import Wikidata, WikidataPrepStage
def adapt_name(version, stage : WikidataPrepStage = WikidataPrepStage.ALMOST_RAW) -> str:
    if version in ('old', 'new'):
        name = 'wikidata_%s_json' % version if stage == WikidataPrepStage.ALMOST_RAW else 'wikidata_%s_prep' % version
    else:
...
def get_direct_types(ent_id : str, version : str) -> list[str]:
    ...
    types = db[name].find_one({'_id' : ent_id}, {'types' : '$claims.P31.mainsnak.datavalue.value.id'})['types']
    return types

def get_subclasses(types : list[str], version :str):
    # Types are entities IDs
    return [y for x in db[adapt_name(version)].find({"_id" : {'$in' : types}}, {"subclasses" : '$claims.P279.mainsnak.datavalue.value.id'}) for y in x.get('subclasses', [])]
...
def wikidata_iterator(version : str, entities_to_ignore : set[str], wikipedia_only=True): 
    ...
    def generator_wikipedia(version : str, entities_to_ignore : set[str]):
        ...
        for x in tqdm.tqdm(db[adapt_name(version, WikidataPrepStage.PREPROCESSED)].aggregate([
            {'$project' : {'_id' : 1}},
            {'$lookup' : {
                'from' : adapt_name(version),
                'localField' : "_id",
    ...
    def generator_full(version : str, entities_to_ignore : set[str]):
        ...
        for x in tqdm.tqdm(db[adapt_name(version)].find({}, {'claims' : 1}), total=total, mininterval=5, file=sys.stdout):
            if x['_id'] in entities_to_ignore:
                continue
            yield x['_id'], x['claims']
