...
import lzma
old_wiki_verb = [json.loads(x) for x in lzma.open(osp.join(path, "verbalizations_old_wikidata.jsonl.xz"), mode='rt', encoding='utf-8')]
wfd_verb = [json.loads(x) for x in lzma.open(osp.join(path, "verbalizations_wfd.jsonl.xz"), mode='rt', encoding='utf-8')]
...
api = HfApi()
api.upload_folder(repo_id=HF_VERB_PATH['path'], folder_path=path_verb_folder, repo_type="dataset", path_in_repo=HF_VERB_PATH['name'])
