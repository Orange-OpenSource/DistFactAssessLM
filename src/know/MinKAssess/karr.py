...
from kb.core import Entity
from know.MinKAssess.src.data_preprocess.gen_obj_alias_clean_dict import judge_obj_in_vocab
from know.MinKAssess.src.data_preprocess.wikidata_get import *
import random
import jsonlines
import os
...
from know.MinKAssess.globs import PROJECT_PATH
from utils.general import download_and_unzip, load_json
try:
...
class KaRR:
    def __init__(self, model, tokenizer, device = 'cuda', thresh=22, custom_rel2alias : dict[str, list[str]] = None, use_aliases=True, additional_aliases=None) -> None:
    ...
    def _load_data(self):
        ...
        self.sub2alias = load_json(f"{PROJECT_PATH}/data/cleaned_T_REx/allsub2alias.json")
        self.obj2rel2rate = load_json(f"{PROJECT_PATH}/data/cleaned_T_REx/obj2rel2rate.json")
        # rel alias filter
        if self.rel2alias is None:
            self.rel2alias = load_json(f"{PROJECT_PATH}/data/relation2template.json")
